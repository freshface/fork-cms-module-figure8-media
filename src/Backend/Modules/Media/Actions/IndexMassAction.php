...
class IndexMassAction extends ActionEdit
{
    ...
    public function execute()
    {
        ...
        if (empty($this->ids)) {
            $this->redirect(Model::createURLForAction('Index') . '&error=no-files-selected');
        }
        ...
        if ($this->action == 'delete') {
            foreach ($this->ids as $id) {
                ...
                $fs = new Filesystem();
                if ($record['filename']) {
                    $fs->remove($files_path . '/' . $record['filename']);
                }
                if ($record['original_filename']) {
                    $fs->remove($files_path . '/' . $record['original_filename']);
                }
                if ($record['filename']) {
                    $fs->remove($preview_files_path . '/' . $record['filename']);
                }

                if ($record['poster_filename']) {
                    $fs->remove($poster_files_path . '/' . $record['poster_filename']);
                }
                if ($record['poster_original_filename']) {
                    $fs->remove($poster_files_path . '/' . $record['poster_original_filename']);
                }
                if ($record['poster_filename']) {
                    $fs->remove($poster_preview_files_path . '/' . $record['poster_filename']);
                }
